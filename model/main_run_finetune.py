...
def objective(trial, args):
    ...
    args.contra_pos_weight = trial.suggest_float("contra_pos_weight", 0.2, 2, log=True)
    args.contra_neg_weight = trial.suggest_float("contra_neg_weight", 1, 4, log=True)
    args.gnn = trial.suggest_categorical("gnn", ["gcn", "gat"])

    set_seed(args.seed)
    param = vars(args)
    args.savedir = os.path.join(args.save_dir, f'drop{args.dropout:.3f}_attresh_{args.att_thresh:.3f}_knn{args.knn}_seed{args.seed}_dims{len(args.enc_dims)}_{args.enc_dims[0]}_recon{args.recon_weight:.3f}_pos{args.contra_pos_weight:.3f}_neg{args.contra_neg_weight:.3f}_net{args.gnn}_k{args.model_k}_rad{args.radius}')
