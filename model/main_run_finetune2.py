...
def objective(trial, args):
    ...
    # args.knn = trial.suggest_int('knn', 4, 6)
    # args.knn = trial.suggest_int('knn', 3, 6)
    args.knn = trial.suggest_categorical('knn', [8, 10, 12, 15])
    ...
    # args.hvg = trial.suggest_categorical('hvg', [10000, 5000, 3000, 2000])

    set_seed(args.seed)
    param = vars(args)
    args.savedir = os.path.join(args.save_dir, f'drop{args.dropout:.3f}_knn{args.knn}_seed{args.seed}_dims{len(args.enc_dims)}_{args.enc_dims[0]}_pos{args.contra_pos_weight:.3f}_neg{args.contra_neg_weight:.3f}_{args.gnn}_k{args.model_k}_hvg{args.hvg}_graphdec')
    ...
    np.save(os.path.join(args.savedir, 'predicted_X.npy'), X_numpy)
