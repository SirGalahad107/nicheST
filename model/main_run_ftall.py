...
import random
import time
from train_all import NicheST_trainer
import optuna
import pandas as pd
...
def objective(trial, args, study):
    
    args.dropout = trial.suggest_float("dropout", 0.1, 0.5)
    # args.att_thresh = trial.suggest_float("att_thresh", 0.001, 0.1, log=True)
    # args.att_heads = trial.suggest_int("att_heads", 1, 4)
    # args.knn = trial.suggest_int('knn', 3, 6)
    args.knn = trial.suggest_categorical('knn', [6, 8, 10, 12, 15, 18])


    # args.enc_dims = trial.suggest_categorical("enc_dims", [[512, 128], [256, 64]])
    # args.dec_dims = trial.suggest_categorical("dec_dims", [[128, 512], [64, 256]])
    args.seed = trial.suggest_int("seed", 1, 1314, step=1)
    enc_dec_pairs = [
        ([512, 128], [128, 512]),
        ([256], [256]),
        ([128], [128]),
        ([512], [512]),
        ([1024], [1024]),
        ([1024, 512, 128], [128, 512, 1024]),
    ]
    args.model_k = trial.suggest_categorical('model_k', [1, 2, 3])
    args.hvg = trial.suggest_categorical('hvg', [2000, 3000])
    # args.radius = trial.suggest_categorical('radius', [40, 50, 60])
    args.enc_dims, args.dec_dims = trial.suggest_categorical("enc_dec_pair", enc_dec_pairs)
    # args.recon_weight = trial.suggest_float("recon_weight", 5, 10)
    args.contra_pos_weight = trial.suggest_float("contra_pos_weight", 0.1, 0.5, log=True)
    args.contra_neg_weight = trial.suggest_float("contra_neg_weight", 1.5, 2)
    args.gnn = trial.suggest_categorical("gnn", ["gcn", "gat", 'gin'])

    # args.embed_type = trial.suggest_categorical("embed_type", ["local", "recon"])
    adata_paths = glob.glob(os.path.join(args.adata_path, '*.h5ad'))
    ...
    for path in adata_paths:
        # slice_str = str(os.path.basename(path))
        
        
        adata = sc.read_h5ad(path)
        adata = filter_adata(adata)
        adata = preprocess(args, adata)
        # adata.uns['file_name'] = slice_str
        param['savedir'] = os.path.join(param['save_dir'], adata.uns['file_name'])

        if not os.path.exists(param['savedir']):
            os.makedirs(param['savedir'])
        ...
        state_dict = nicheST_trainer.train(feature_list, edge_ind_list, sub_node_sample_list, sub_edge_ind_sample_list)
        ...
        # cluster_labels[slice_str] = cluster_labels
        ...
        # new_labels[slice_str] = new_label
        ...
        del feature_list
        del edge_ind_list
        del sub_node_sample_list
        del sub_edge_ind_sample_list
        gc.collect()
        torch.cuda.empty_cache()

    mean_ari = np.mean(total_aris)

    try:
        best_mean_ari = mean_ari > study.best_value
    except ValueError:
        best_mean_ari = True
    if best_mean_ari:
        for idx, adata in enumerate(processed_adatas):
            args.savedir = os.path.join(args.save_dir, adata.uns['file_name'])
            # adata.write(os.path.join(args.savedir, f'{adata.uns['file_name']}.h5ad'))
            adata.write(os.path.join(args.savedir, f"{adata.uns['file_name']}.h5ad"))
            ...
            all_categories = true_labels_cat.categories
            ari = adjusted_rand_score(true_color_indices, pred_color_indices)
            torch.save(model_state_dicts[idx], os.path.join(args.savedir, 'model_weights.pth'))
            plot_ari_clustering(adata, true_color_indices, pred_color_indices, all_categories, ari, args.savedir)

        args_dict = vars(args)
        np.save(os.path.join(args.save_dir, "args.npy"), args_dict)

        np.save(os.path.join(args.save_dir, 'total_aris.npy'), total_aris)
        
        with open(os.path.join(args.save_dir, "info.txt"), "a") as f:
            f.write(f"mean_ari: {mean_ari}\n")
...
def main():
    ...
    study.optimize(lambda trial: objective(trial, args, study), n_trials=1000, n_jobs=1)

    print("Best Mean ARI score:", study.best_value)
